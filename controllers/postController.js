...
module.exports.createPost = (req, res) => {
  ...
  form.parse(req, async (error, fields, files) => {
    ...
    const errors = [];
    if (title === "") {
      errors.push({ msg: "Yazı gerekli" });
    }
    if (body === "") {
      errors.push({ msg: "Başlık gerekli" });
    }
    if (description === "") {
      errors.push({ msg: "Açıklama gerekli" });
    }
    if (slug === "") {
      errors.push({ msg: "" });
    }
    if (Object.keys(files).length === 0) {
      errors.push({ msg: "resim gerekli" });
    } else {
...
module.exports.updateValidations = [
  body("title").notEmpty().trim().withMessage("yazı gerekli"),
  body("body")
    .notEmpty()
    .trim()
    .custom((value) => {
    ...
    .withMessage("gerekli"),
  body("description").notEmpty().trim().withMessage("açıklama gerekli"),
];
...
module.exports.updateImage = (req, res) => {
  ...
  form.parse(req, (errors, fields, files) => {
    ...
    const imageErrors = [];
    if (Object.keys(files).length === 0) {
      imageErrors.push({ msg: "resim yükleyin" });
    } else {
      const { type } = files.image;
      const split = type.split("/");
      const extension = split[1].toLowerCase();
...
module.exports.deletePost = async (req, res) => {
	...
	try {
		...
		return res.status(200).json({ msg: 'Silindi' });
	} catch (error) {
		return res.status(500).json({ errors: error, msg: error.message });
	}
};